...
function writeAllOtherTsFiles(): Result<void> {
  ...
  result = writeFile(fileNames.defaultTheme, defaultThemeSrc);
  if (result.tag === "err") return result;
  ...
  result = writeFile(fileNames.themeDefiniton, themeDefinitonSrc);
  if (result.tag === "err") return result;

  const pjsonResult = readPackageJSON();
  if (pjsonResult.tag === "err") return result;

  const {
    data: { type = "commonjs" },
  } = pjsonResult;

  return writeFile(fileNames.genScript(type), genScriptSrc);
}
...
const fileNames = {
  ...
  genScript: (type: "commonjs" | "module") =>
    `genScript.${type === "commonjs" ? "mjs" : "js"}`,
} as const;

const defaultCommentString = `// This file was autogenerated.
// any changes made will be overidden the next time it is regenerated.`;
