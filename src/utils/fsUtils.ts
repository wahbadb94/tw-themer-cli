...
`;
  ...
  // check if we are in a commonjs or esm (module) context.
  const pjsonResult = readPackageJSON();
  if (pjsonResult.tag === "err") return pjsonResult;
  const { type = "commonjs" } = pjsonResult.data;

  // write all the other files that dependend on this file.
  return writeAllOtherTsFiles(type);
}
...
import genScriptSrc from "../fileContents/_autogenerated/genScript.js";
import { cwd } from "process";
import { ProjectType } from "../types/projectType.js";

function writeAllOtherTsFiles(type: ProjectType): Result<void> {
  let result = writeFile(fileNames._autogeneratedIndex, genIndexSrc(type));
  if (result.tag === "err") return result;

  result = writeFileIfNotExists(
  ...
  result = writeFile(fileNames.derived, derivedSrc(type));
  if (result.tag === "err") return result;

  result = writeFile(fileNames.gen, genSrc(type));
  if (result.tag === "err") return result;

  result = writeFile(fileNames.serialize, serializeSrc(type));
  if (result.tag === "err") return result;

  result = writeFile(fileNames.typesIndex, typesIndexSrc(type));
  if (result.tag === "err") return result;
  ...
  return writeFile(fileNames.genScript, genScriptSrc(cwd(), type));
}
...
function writeFileIfNotExists(
  ...
  const pathStr = path.join(getTwdDirPath(), pathRelativeToBaseDir);
  if (fileExists(pathStr)) return result.ok(undefined);

  return writeFile(pathRelativeToBaseDir, src, commentString);
}
